...
def _find_earthreason_triplets(base_image_dir, ds_name, split,
...
class EarthReasonDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        precision: str = "fp32",
        image_size: int = 224,
        num_classes_per_sample: int = 1,   # EarthReason is one-target-per-image
        exclude_val=False,
        earth_reason_data="EarthReason|train",
        ...
        dataset_name, split = earth_reason_data.split("|")
        splits = split.split("_")

        images, labels, qas = [], [], []
        for sp in splits:
            sp_images, sp_labels, sp_qas = _find_earthreason_triplets(
        ...
        print("number of EarthReason samples: ", len(images))
...
class EarthReasonValDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        tokenizer,
        vision_tower,
        val_dataset,  # e.g. "EarthReason|val"
        image_size=1024,
        seg_token_num=1,
        ...
        images, labels, qas = _find_earthreason_triplets(
            base_image_dir, ds_name, split, images_dirname, labels_dirname, qas_dirname
        )
        self.images = images
        self.labels = labels
        self.qas = qas
        self.data_type = "earth_reason"

        print(f"number of EarthReason {split} samples: ", len(images))
